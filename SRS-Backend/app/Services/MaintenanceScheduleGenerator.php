...
class MaintenanceScheduleGenerator
{
    ...
    public function preview(int $year, int $month): array
    {
        ...
        foreach ($trains as $trainId) {
            if ($trainId === '04') {
                ...
                if ($date) $this->put($plan, $date, $trainId, 'A+C');
                continue;
            }

            $trainPast = $past->where('train_id', $trainId);
            $this->planBCycle($trainId, $trainPast, $start, $end, $plan, $blocked, $holidays, $warnings);
            ...
            if ($lastC) {
                foreach ($this->dueDates($lastC, self::C_TARGET, $start, $end, $holidays) as $due) {
                    ...
                    if ($date) $this->put($plan, $date, $trainId, 'C');
                }
            } else {
    ...
    private function planAVisits(array $trains, Collection $past, Carbon $start, Carbon $end, array &$plan, array $blocked, array $holidays, array &$warnings): void
    {
        ...
        foreach ($candidates as [$due, $trainId]) {
            ...
            if ($date) $this->put($plan, $date, $trainId, 'A');
            else $warnings[] = "Train {$trainId}: no free A slot within the 13-17 day balance window.";
        }
    }

    private function planBCycle(string $trainId, Collection $past, Carbon $start, Carbon $end, array &$plan, array $blocked, array $holidays, array &$warnings): void
    {
        $last = $past->filter(fn ($row) => in_array($this->baseCode($row->code), self::B_CYCLE, true))->last();
        if (!$last) {
            $warnings[] = "Train {$trainId}: no B-cycle history was found; B/G was not generated.";
            return;
        }
        ...
        if (!$due->betweenIncluded($start, $end)) return;
        ...
        if ($nextCode === 'G') {
            ...
            while ($second->lte($end) && !$this->isWorkingDay($second, $holidays)) $second->addDay();
            if ($second->lte($end) && empty($plan[$second->toDateString()][$trainId])) $this->put($plan, $second, $trainId, 'G');
        }
    }
    ...
    private function planNineYearWork(Carbon $start, Carbon $end, Collection $past, array $trains, array $holidays, array &$plan, array &$generatedMeta, array &$warnings): array
    {
        ...
        if (!$latest || $latest->schedule_date->lt($start->copy()->subDays(60))) return $blocked;
        ...
        for ($i = $allDates->count() - 2; $i >= 0; $i--) {
            if ($allDates[$i]->diffInDays($blockStart) > 60) break;
    ...
    private function dueDates(Carbon $last, int $interval, Carbon $start, Carbon $end, array $holidays): array
    {
        ...
        while (true) {
            ...
            while ($count < $interval) {
                $cursor->addDay();
                if ($this->isWorkingDay($cursor, $holidays)) $count++;
            }
            if ($cursor->gt($end)) break;
            if ($cursor->gte($start)) $dates[] = $cursor->copy();
        }
        return $dates;
    }
    ...
    private function balancedADate(Carbon $due, Carbon $start, Carbon $end, string $trainId, array $plan, array $blocked, array $holidays): ?Carbon
    {
        foreach ([0, -1, 1, -2, 2] as $offset) {
            $candidate = $this->shiftWorkingDays($due, $offset, $holidays);
            if (!$candidate->betweenIncluded($start, $end)) continue;
            if (!empty($blocked[$trainId][$candidate->toDateString()]) || !empty($plan[$candidate->toDateString()][$trainId])) continue;
            $aCount = collect($plan[$candidate->toDateString()] ?? [])->filter(fn ($code) => $code === 'A')->count();
            if ($aCount < self::MAX_A_PER_DAY) return $candidate;
        }
        return null;
    }
    ...
    private function nearestAvailable(Carbon $due, Carbon $start, Carbon $end, string $trainId, array $plan, array $blocked, array $holidays, bool $forwardOnly = false): ?Carbon
    {
        ...
        foreach ($offsets as $offset) {
            ...
            if (!$candidate->betweenIncluded($start, $end) || !$this->isWorkingDay($candidate, $holidays)) continue;
            if (empty($blocked[$trainId][$candidate->toDateString()]) && empty($plan[$candidate->toDateString()][$trainId])) return $candidate;
        }
        return null;
    }

    private function shiftWorkingDays(Carbon $date, int $offset, array $holidays): Carbon
    {
        if ($offset === 0) return $date->copy();
        $cursor = $date->copy();
    ...
    private function holidayDates(Carbon $start, Carbon $end): array
    {
        ...
        PublicHoliday::query()->whereDate('date', '<=', $end)->where(function ($query) use ($start) {
            ...
            foreach (CarbonPeriod::create($holiday->date, $holiday->end_date ?? $holiday->date) as $date) $dates[$date->toDateString()] = true;
        });
        return $dates;
    }
    ...
    {
        foreach (self::B_CYCLE as $candidate) if (str_contains(strtoupper($code), $candidate)) return $candidate;
