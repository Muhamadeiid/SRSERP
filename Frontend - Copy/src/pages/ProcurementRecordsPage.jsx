...
import { Building2, Calculator, ClipboardX, FileSpreadsheet, Loader2, Plus, RefreshCw, Search, Star, X } from 'lucide-react'
import {
...
export default function ProcurementRecordsPage() {
  ...
  return <div className="p-4 sm:p-6 space-y-5">
    ...
    <div className="flex flex-wrap gap-2 border-b border-neutral-200">
      {tabs.map(([key, label, Icon]) => <button key={key} onClick={() => { setTab(key); setSearch('') }} className={`flex items-center gap-2 px-4 py-3 text-xs font-bold border-b-2 ${tab === key ? 'border-primary text-primary' : 'border-transparent text-neutral-400'}`}><Icon className="w-4 h-4" />{label}</button>)}
    </div>
    ...
    {loading ? <div className="py-20 flex justify-center"><Loader2 className="w-7 h-7 animate-spin text-primary" /></div> : <>
      ...
      {tab === 'budgets' && <section className="space-y-4">
        {canManage && <button onClick={()=>setBudgetForm({ year:new Date().getFullYear(), month:new Date().getMonth()+1, items:[emptyBudgetItem()] })} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-white text-xs font-bold"><Plus className="w-4 h-4" />New Monthly Plan</button>}
